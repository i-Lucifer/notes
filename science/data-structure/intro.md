...
| 结构 | 物理结构 | 查找 | 更新 | 插入 | 删除 |
| ---- | -------- | ---- | ---- | ---- | ---- |
| 数组 | 顺序存储 | O(1) | O(1) | O(n) | O(n) |
| 链表 | 随机存储 | O(n) | O(1) | O(1) | O(1) |

- 特点对比

| 结构 | 优点           | 缺点                         | 特点                        | 适合场景                          | 直接应用                | 相关算法          |
| ---- | -------------- | ---------------------------- | --------------------------- | --------------------------------- | ----------------------- | ----------------- |
| 数组 | 随机访问高效   | 插入删除要移动数据，影响效率 | 顺序存储 只记录首地址和长度 | 读多写少                          | 语言的string与[]byte    | 二分查找 O log(n) |
| 链表 | 灵活插入和删除 | 查找数据麻烦                 | 不考虑扩容，利用零碎内存    | 写多读少</br>双端读写，低深度访问 | redis的list go的channel |                   |
...
| 插入 中间   | 先挪移，再插入                                               | n      |
| 插入 超范围 | 先扩容，再插入，</br>扩容=申请内存+复制旧数据</br>小容量采取翻倍扩容，大容量可以1.25倍扩容(golang的切片) | n      |
| 删          | 与插入相反                                                   | n      |
